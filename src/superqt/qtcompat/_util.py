...
base = None
try:
    from PyQt6.sip import wrappertype as base
except ImportError:
    try:
        from shiboken6.Shiboken import Object as base
    except ImportError:
        pass

if base is None:
    ScopedEnumMeta = object
else:

    class ScopedEnumMeta(base):  # type: ignore
        def __new__(mcls, name, bases, attrs, **kwargs):
            obj = getattr(import_module(attrs["__module__"]), attrs["__qualname__"])
            cls = base.__new__(mcls, name, (obj,), attrs)
            cls._enums = {}
        ...
        def __getattr__(cls, key):
            ...
            raise AttributeError(
                f"type object { cls.__name__!r} has no attribute {key!r}"
            )
